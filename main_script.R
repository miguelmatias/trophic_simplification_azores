...
sst_jiang <- read.table("data/table_sst_jiang.csv", header = TRUE, sep = ",", dec = ".")
sst_abrantes <- read.table("data/table_sst_abrantes.csv", header = TRUE, sep = ",", dec = ".")
...
# ------------------------------------------------------------------------------
# Combine hierarchical clustering outputs from all lakes into a single dataframe
# ------------------------------------------------------------------------------

data_combined <- ls_all_hclusts_complete_paleo %>%
  map_dfr(as.data.frame)  # Combine list elements into one long data frame

# ------------------------------------------------------------------------------
# Compute time range per lake and order lakes from longest to shortest
# ------------------------------------------------------------------------------
...
lake_ranges <- data_combined %>%
  ...
  arrange(desc(data_range))  # Order lakes by longest time coverage

lake_levels <- lake_ranges$lake  # Store ordered lake names

# ------------------------------------------------------------------------------
# Update factor levels and cluster assignments in the combined data
# ------------------------------------------------------------------------------

data_combined <- data_combined %>%
  mutate(
    lake = factor(lake, levels = lake_levels),          # Set lake as ordered factor
    amd_clusts = as.factor(amd_clusts)                  # Convert cluster assignments to factor
  )

# ------------------------------------------------------------------------------
# Prepare rectangle overlay data to mask out areas beyond each lake's range
# ------------------------------------------------------------------------------

rect_data <- lake_ranges %>%
  select(lake, min_age, max_age) %>%
  mutate(
    lake = factor(lake, levels = lake_levels),  # Retain lake factor level order
    xmin1 = 0,                                  # Left trim range
    xmax1 = min_age,
    xmin2 = max_age,                            # Right trim range
    xmax2 = 2010
  ) %>%
  pivot_longer(
    cols = c(xmin1, xmin2),
  ...
  filter(substr(rect_part, 5, 5) == substr(rect_part2, 5, 5)) %>%  # Match xmin-xmax pairs
  select(lake, xmin, xmax) %>%
  mutate(
    ymin = -Inf,  # Full y-range for blocking
    ymax = Inf
  )

# ------------------------------------------------------------------------------
# Create final ggplot showing relative abundance of trophic clusters over time
# ------------------------------------------------------------------------------

plot_density_fclusts_time_lines_1B <- 
  ggplot(data_combined, aes(x = age_ce, fill = amd_clusts)) +
  facet_wrap(~ lake, ncol = 1, scales = "fixed", strip.position = "right") +  # One column per lake

  # Plot kernel density estimate of cluster proportions
  geom_density(
    adjust = 1.5,
    position = "fill",
    colour = "white"
  ...
  # Overlay white rectangles to mask out time intervals not covered by data
  geom_rect(
    data = rect_data,
  ...
  # Add vertical dashed lines for highlighted dates
  geom_vline(
  ...
  # Color scale for trophic clusters
  ...
  # Y-axis as relative proportion (0 to 1)
  scale_y_continuous(
    breaks = c(0, 0.5, 1),
    labels = c("0", "0.5", "1")
  ) +

  # Minimalist theme with layout adjustments
  theme_minimal() +
  theme(
    ...
    plot.margin = unit(c(2, 1, 1, 1), "cm")  # Add space for facet labels
  ) +
...
# ------------------------------------------------------------------------------
# Prepare and interpolate SST Jiang
# ------------------------------------------------------------------------------

sst_jiang_interpolated <- approx(
  x = sst_jiang$age_ce,
  y = sst_jiang$SST.DTF,
  xout = interp_years,
  method = "linear"
...
sst_jiang <- as.data.frame(sst_jiang_interpolated)
colnames(sst_jiang) <- c("age_ce", "SST.DTF")

# ------------------------------------------------------------------------------
# Prepare and interpolate and normalize SST Abrantes
# ------------------------------------------------------------------------------

sst_abrantes_interpolated <- approx(
  x = sst_abrantes$age_ce,
  y = sst_abrantes$SST,
  xout = interp_years,
  method = "linear"
)

sst_abrantes_interpolated$y <- (
  sst_abrantes_interpolated$y - min(sst_abrantes_interpolated$y, na.rm = TRUE)
) / (
  max(sst_abrantes_interpolated$y, na.rm = TRUE) - min(sst_abrantes_interpolated$y, na.rm = TRUE)
)

sst_abrantes <- as.data.frame(sst_abrantes_interpolated)
colnames(sst_abrantes) <- c("age_ce", "SST")

sst_abrantes_binned <- sst_abrantes %>%
  mutate(bin_50yr = floor(age_ce / 50) * 50) %>%
  group_by(bin_50yr) %>%
  summarise(avg_value = mean(SST, na.rm = TRUE), .groups = "drop")
...
# --------------------------------------
# Plot 3: Sea Surface Temperature (SST) reconstruction from sst_abrantes
# --------------------------------------
sst_abrantes_plot <- ggplot(sst_abrantes, aes(x = age_ce, y = SST)) +
  ...
  # geom_line(data = sst_abrantes_binned, aes(x = bin_50yr, y = avg_value),
  #           color = "steelblue", linewidth = 2) +
