...
# ------------------------------------------------------------------------------
pollen <- read.table("data/table_pollen_counts_azores.csv", header = TRUE, sep = ",", dec = ".")

# ------------------------------------------------------------------------------
# Load vegetation turnover model output (regional smooth)
# ------------------------------------------------------------------------------
hgam <- read.csv("data/table_azores_HGAM_output.csv") %>%
  filter(is.na(sitename)) %>%
  mutate(age = 1950 - age)  # Convert from BP to CE
...
joined_df <- cbind(age_ce = norm_abund_guilds$age_ce, comm_matrix) %>%
  ...
  # left_join(sst_abrantes, by = "age_ce") %>%  # optional if dataset is needed
...
# --------------------------------------------
lake_centers <- data.frame(island = lake_metadata$lake, lat = env$lat, lon = env$long)
