...
for (i in 1:length(ls_dca_diatoms_2)) {
  ls_dca_diatoms_2[[i]] <- bind_cols(ls_df_codes_diat[[i]], ls_dca_diatoms_2[[i]])
}
...
for (i in 1:length(ls_dca_chiro_2)) {
  ls_dca_chiro_2[[i]] <- bind_cols(df_chiro_codes[[i]], ls_dca_chiro_2[[i]])
}
...
## 3. Lake-scale proportions trophic structures 
```{r fig.asp = 1, fig.width = 12}
...
lakes_col_groups <- scico(9, begin = 0, end = 0.95, palette = 'roma')

# "#7E1700" "#9B5517" "#B3862E" "#CCBF63" "#C8E9B6" "#8CDCD6" "#46ACCC" "#287CB9" "#1549A2"
...
(p.DCA.gl + p.DCA.gl.dev) + plot_annotation(title = "Archipelago-scale vegetation turnover")
...
DF_to_groups <- df_wk %>%
  filter(age_ce > 0, group == "Herb") %>%
  mutate(p_anomaly = percent_anomaly, sitename = factor(sitename)) %>%
  droplevels()

mod_to_groups <- gam(percentage ~ s(age_ce, bs = "tp", k = 30) +
                       s(age_ce, by = sitename, k = 10, m = 1, bs = "tp") +
                       s(sitename, bs = "re"),
                     data = DF_to_groups, method = "REML")

summary(mod_to_groups)
k.check(mod_to_groups, n.rep = 500)
appraise(mod_to_groups)

sm_to_groups <- smooth_estimates(mod_to_groups) %>% add_confint()
partial_residuals_df_groups <- DF_to_groups %>% add_partial_residuals(mod_to_groups)

lakes_col <- scico(n_distinct(DF_to_groups$sitename), begin = 0, end = 0.95, palette = 'roma')

p.gl <- sm_to_groups %>%
  filter(.smooth == "s(age_ce)") %>%
  ggplot() +
  geom_rug(aes(x = age_ce), data = partial_residuals_df_groups, sides = "b", length = unit(0.02, "npc")) +
  geom_ribbon(aes(x = age_ce, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.2) +
  geom_point(aes(x = age_ce, y = `s(age_ce)`, color = sitename), data = partial_residuals_df_groups, alpha = 0.8, size = 1.5) +
  geom_line(aes(x = age_ce, y = .estimate), linewidth = 1) +
  labs(y = "Percentage", x = "Age (CE)") +
  scale_color_manual("Lake", values = lakes_col) +
  theme_minimal(base_size = 7) +
  scale_x_continuous(n.breaks = 10) +
  theme(panel.grid.minor.y = element_blank(), legend.position = "bottom")

mod_to_dev <- derivatives(mod_to_groups, select = "s(age_ce)", eps = 1, level = 0.9, interval = "simultaneous", seed = 111)
