...
# Load lake metadata
# ------------------------------------------------------------------------------
lake_metadata <- readxl::read_excel(path = "data/table_lake_metadata.xlsx")  # lake coordinates, area, etc.
...
# ----------------------------------------------------------
# 1. Load and clean pollen data
# ----------------------------------------------------------

df_1 <- pollen

# Remove aquatic vegetation and unidentified taxa, then group by ecological type
df_wk <- df_1 %>%
  filter(ecologicalgroup != "UNID") %>%
  filter(!ecologicalgroup %in% c("AQVP", "AQBR")) %>%
  group_by(ID, sitename, age_ce, group) %>%
  summarise(value = sum(value), .groups = 'drop')

# Calculate group percentage per sample
df_wk <- df_wk %>%
  group_by(ID) %>%
  mutate(percentage = value / sum(value) * 100) %>%
  ungroup()

# Mean percentage per lake/group combination
mean_percentage <- df_wk %>%
  group_by(sitename, group) %>%
  summarise(mean_percentage = mean(percentage), .groups = 'drop')

# Add anomaly and drop mean
df_wk <- df_wk %>%
  left_join(mean_percentage, by = c("sitename", "group")) %>%
  mutate(percent_anomaly = percentage - mean_percentage) %>%
  select(-mean_percentage) %>%
  droplevels()

# Set color palettes
sites <- length(unique(df_wk$sitename))
eco_gr <- length(unique(df_wk$group))

sites_col <- scico(sites, begin = 0, end = 0.95, palette = 'roma', categorical = FALSE)
gr_col <- scico(eco_gr, begin = 0, end = 0.95, palette = 'nuuk', categorical = FALSE)
...
df_wk_dca <- df_1 %>%
  filter(ecologicalgroup != "UNID", age_ce > 0, !sitename %in% selection) %>%
  droplevels()

Sites <- sort(unique(df_wk_dca$sitename))
results_list <- list()

for (i in seq_along(Sites)) {
  df.sp <- df_wk_dca %>%
    filter(sitename == Sites[i]) %>%
    select(ID, value, taxa) %>%
    group_by(ID, taxa) %>%
    summarise(value = sum(value), .groups = 'drop') %>%
...
DCA.res <- Reduce(full_join, results_list) %>%
  ...
  mutate(DCA1 = case_when(
    sitename == 'Azul' ~ DCA1 * -1,
    sitename == 'Gingal' ~ DCA1 * -1,
    TRUE ~ DCA1
  ))
...
mod_to <- gam(DCA1 ~ s(age_ce, bs = "tp", k = 25) +
                s(age_ce, by = sitename, k = 15, m = 1, bs = "tp") +
                s(sitename, bs = "re"),
...
mod_to_dev <- derivatives(mod_to, select = "s(age_ce)", eps = 1, level = 0.9,
                          interval = "simultaneous", seed = 22)

color_mapping <- scico(2, begin = 0, end = 0.9, palette = 'vik', categorical = FALSE)

p.DCA.gl.dev <- mod_to_dev %>%
  add_sizer(type = "change") %>%
  ggplot() +
  geom_ribbon(aes(x = age_ce, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.2) +
  geom_line(aes(x = age_ce, y = .derivative, color = (.lower_ci > 0 | .upper_ci < 0),
                linewidth = (.lower_ci > 0 | .upper_ci < 0)), show.legend = FALSE) +
  scale_discrete_manual("linewidth", values = c(0.5, 1.5)) +
  scale_color_manual(values = color_mapping) +
  labs(y = "Derivative", x = "Age (CE)") +
  theme_minimal(base_size = 9) +
  scale_x_continuous(n.breaks = 10) +
  theme(panel.grid.minor.y = element_blank())

library(patchwork)
p <- p.DCA.gl + p.DCA.gl.dev + plot_annotation(title = "Archipelago-scale vegetation turnover")
print(p)

# ----------------------------------------------------------
# 4. HGAM on Trees or Herbs
# ----------------------------------------------------------

DF_to_groups <- df_wk %>%
  filter(sitename %in% unique(df_wk$sitename),
         age_ce > 0,
         group == "Herb") %>%
  mutate(p_anomaly = percent_anomaly,
         sitename = factor(sitename)) %>%
  droplevels()

variable <- "Percentage"
sites <- n_distinct(DF_to$sitename)
lakes_col <- scico(sites, begin = 0, end = 0.95, palette = 'roma')

mod_to_groups <- gam(percentage ~ s(age_ce, bs = "tp", k = 30) +
                s(age_ce, by = sitename, k = 10, m = 1, bs = "tp") +
                s(sitename, bs = "re"),
              data = DF_to, method = "REML")
...
sm_to_groups <- smooth_estimates(mod_to) |> add_confint()
partial_residuals_df <- DF_to_groups |> add_partial_residuals(mod_to)

p.gl <- sm_to_groups |>
  filter(.smooth == "s(age_ce)") |>
  ggplot() +
  geom_rug(aes(x = age_ce), data = partial_residuals_df, sides = "b", length = unit(0.02, "npc")) +
  geom_ribbon(aes(x = age_ce, ymin = .lower_ci, ymax = .upper_ci), alpha = 0.2) +
  geom_point(aes(x = age_ce, y = `s(age_ce)`, color = sitename), data = partial_residuals_df, alpha = 0.8, size = 1.5) +
  geom_line(aes(x = age_ce, y = .estimate), linewidth = 1) +
  labs(y = variable, x = "Age (CE)") +
  scale_color_manual("Lake", values = lakes_col) +
...
mod_to_dev <- derivatives(mod_to, select = "s(age_ce)", eps = 1, level = 0.9,
                          interval = "simultaneous", seed = 111)
color_mapping <- scico(2, begin = 0, end = 0.9, palette = 'vik', categorical = FALSE)
...
# Final combined plot
(p.gl + p.gl.dev) + plot_annotation(title = "Trends in the relative abundance of Herbs in Azores")
...
# ------------------------------------------------------------------------------
# Interpolate and normalize SST Abrantes
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Interpolate NH summer temperatures (Büntgen et al.)
# ------------------------------------------------------------------------------
...
## Align datasets using 30y windows
...
# ------------------------------------------------------------------------------
# Final RDA biplot figure (Revised)
# ------------------------------------------------------------------------------
...
# -----------------------------------------------
tree_grob_paleo <- grid::rasterGrob(png::readPNG("figures/decision_tree_paleo.png"))
