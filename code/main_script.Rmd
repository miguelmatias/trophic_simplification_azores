...
# TROPHIC STRUCTURES ANALYSIS - PALEO ONLY
## Get clusters 
...
norm_abund <- 
  bind_cols(
    select(tab_sums, lake:age_ce),
    select(tab_sums, high_profile:euplanktonic)/rowSums(select(tab_sums, high_profile:euplanktonic)),
    select(tab_sums, algivore:last_col())/rowSums(select(tab_sums, algivore:last_col()))
    ) %>% 
  replace(is.na(.), 0)
...
ls_opt_clusts <-
   ...
   PlotoptAMDclusters(., .iterations = 1000, .min_clusts = 3, .num_groups = 9)
...
# Define threshold
threshold <- 0.01  # Change this based on your data's scale

# Find the first point where the rate of change is below the threshold
asymptote_point <- which(abs(dy) < threshold)[1]

# Plot the data and highlight the asymptote point
...
# Create the mapping from cluster numbers to their ranks
cluster_mapping <- key %>% 
  select(amd_clusts, rank) %>% 
  deframe()  # Creates a named vector with amd_clusts as names and rank as values

# Replace amd_clusts in global_matrix_merged_paleo with ranks
...
# split master table into list of dataframes for each selected lake
...
merged_data <- ls_all_hclusts_complete_paleo %>%
  map(~.x %>% as.data.frame()) %>%
  bind_rows() %>%
  #left_join(., select(ungroup(df_fgroups_glob_div), -lake), by = "core_depth_id") %>% 
  filter(age_ce >0) %>% 
...
# Generate compact letter display for significant groups
cld <- multcompView::multcompLetters4(anova_res, tukey_res)

# Extract letters and create the `signif_letters` data frame
signif_letters <- data.frame(
  amd_clusts = names(cld$amd_clusts$Letters),
  letters = cld$amd_clusts$Letters
)

# Ensure the significance letters data has a y-position for plotting
signif_letters <- signif_letters %>%
  ...
  mutate(y_position = max_y * 1.1)  # Adjust position above the boxplots

# Plot with boxplots and significant group letters
box_plot_clusts <- ggplot(merged_data, aes(x = amd_clusts, y = total_nspp_by_lake_core)) +
  ...
  ggpubr::stat_compare_means(method = "anova", label.y = 9, cex = 4) + # Add global p-valu
...
# Perform ANOVA and Tukey HSD per lake
signif_letters_per_lake <- merged_data %>%
  group_by(lake) %>%
  nest() %>%
  mutate(
    # Ensure `amd_clusts` is a factor and count levels
    data = map(data, ~ .x %>% mutate(amd_clusts = as.factor(amd_clusts))),
    num_levels = map_int(data, ~ n_distinct(.x$amd_clusts)),
    # Perform ANOVA only for lakes with sufficient levels
    anova_res = map2(data, num_levels, ~ {
      if (.y > 1) {
        tryCatch(aov(total_nspp_by_lake_core ~ amd_clusts, data = .x), error = function(e) NA)
      } else {
        NA
      }
    }),
    # Tukey HSD only for valid ANOVA results
    tukey_res = map(anova_res, ~ {
      if (is.list(.x)) {
        tryCatch(TukeyHSD(.x), error = function(e) NA)
      } else {
        NA
      }
    }),
    # Extract letters for significant differences
    cld = map2(anova_res, tukey_res, ~ {
      if (is.list(.x) && is.list(.y)) {
        tryCatch(multcompView::multcompLetters4(.x, .y)$amd_clusts$Letters, error = function(e) "N/A")
      } else {
        "N/A"
      }
    })
  ...
  select(lake, cld) %>%
  unnest_wider(cld, names_sep = "_") %>%
  pivot_longer(
    cols = starts_with("cld_"),
    names_to = "amd_clusts",
    values_to = "letters"
  ) %>%
  mutate(
    amd_clusts = str_remove(amd_clusts, "cld_"),
    letters = ifelse(letters == "N/A", "N/A (Not enough levels)", letters)
  )

# Display results
print(signif_letters_per_lake)

# Merge with maximum y-values for positioning
signif_letters_per_lake <- signif_letters_per_lake %>%
  left_join(
    merged_data %>%
      group_by(lake, amd_clusts) %>%
      summarise(max_y = max(total_nspp_by_lake_core, na.rm = TRUE), .groups = "drop"),
    by = c("lake", "amd_clusts")
  ) %>%
  mutate(y_position = max_y * 1.1)  # Adjust position above the boxplots

# Ensure all combinations of lake and amd_clusts exist
merged_data_complete <- merged_data %>%
  complete(lake, amd_clusts = factor(1:5, levels = 1:5), fill = list(total_nspp_by_lake_core = NA))

# Create the plot with the updated data
box_plot_clusts_2 <- ggplot(merged_data_complete, aes(x = amd_clusts, y = total_nspp_by_lake_core)) +
  geom_boxplot(aes(fill = amd_clusts), na.rm = TRUE) +
  facet_wrap(~lake, scales = "free_x") +
  scale_x_discrete(limits = as.character(1:5)) + # Explicitly set x-axis limits
  scale_fill_viridis_d(name = "Community Trophic \nstructures", option = "viridis", direction = -1) +
  theme_minimal() +
  labs(
    x = "Community Trophic Structures",
...
diversity_over_time_lines <- 
  ls_all_hclusts_complete_paleo %>%
  map(~.x %>% as.data.frame()) %>% 
  bind_rows() %>%
  left_join(., df_fgroups_glob_div) %>% 
  #left_join(., df_age_model, by = "core_depth_id") %>% 
  ggplot(aes(x = age_ce, y = total_nspp_by_lake_core)) +
  facet_wrap(~lake, scales = "free_x") +
  geom_smooth(method = "gam") +
  geom_point(aes(color = amd_clusts)) +
  scale_colour_viridis_d(name = "Community Trophic \nstructures", option = "viridis", direction = -1) +
  theme(plot.background = NULL) +
  theme_minimal() +
  theme(
...
diversity_over_time_lines <- 
  ...
  group_by(lake) %>%
  mutate(
    standardized_diversity = scale(total_nspp_by_lake_core)[, 1]
  ) %>%
  ungroup() %>%
  ggplot(aes(x = age_ce, y = standardized_diversity)) +
  facet_wrap(~lake, ncol = 1, scales = "fixed") +
...
# Define colors for each facet group
facet_colors <- c(
...
# Explicitly list the column names in `select()`
boxplots <- 
  bind_cols(global_matrix_merged_paleo, select(norm_abund, c(euplanktonic, high_profile, low_profile, motile, algivore, detritivore, plantivore, predator))) %>%
  pivot_longer(cols = c(euplanktonic, high_profile, low_profile, motile, algivore, detritivore, plantivore, predator)) %>%
  mutate(
    amd_clusts = as.factor(amd_clusts),
    # Reorder the `name` factor to your specified sequence
    name = fct_relevel(
      name,
      "euplanktonic", "high_profile", "low_profile", "motile", "algivore", "detritivore", "plantivore", "predator"
    )
  ) %>% 
  ...
  facet_wrap2(
    ...
    strip = strip_themed(
      text_x = element_text(color = "black", face = "plain"),
      background_x = list(
...
plot_density_fclusts_time_lines <- 
  ls_all_hclusts_complete_paleo %>%
  map(~.x %>% as.data.frame()) %>% 
  bind_rows() %>%
  ggplot(aes(amd_clusts)) +
  facet_wrap(~lake, scales = "free")+
  geom_density(aes(x = age_ce, color = as.factor(amd_clusts)), size = 2) +
  scale_colour_viridis_d(name = "Community Trophic \nstructures", option = "viridis", direction = -1) +
      theme(plot.background = NULL) +
      theme_minimal() +
      theme(
...
```{r fig.asp = 1, fig.width = 12}
plot_density_fclusts_time_lines_1 <- 
  ls_all_hclusts_complete_paleo %>%
  map(~.x %>% as.data.frame()) %>% 
  bind_rows() %>%
  ggplot(aes(x=age_ce, fill = as.factor(amd_clusts))) +
  facet_wrap(~lake, scales = "free")+
  ...
  scale_fill_viridis_d(name = "Community Trophic \nstructures", option = "viridis", direction = -1) +
      ...
      theme_minimal() +
      theme(
        axis.title.y = element_text("Proportion of Community Trophic Structures"),
        plot.title = element_text(size = 12),
        legend.background = element_rect(fill = "white", linewidth = 4, colour = "white"),
        legend.justification = c(0, 1),
        # legend.position = c(0, 1),
...
# Set PNG output for saving the tree plot
png("../figures/decision_tree_paleo.png", width = 7.29, height = 4.51, units = "in", res = 600)

# Generate the plot for export
rpart.plot::rpart.plot(
  decision_tree_paleo,
  type = 5,
  extra = 2,
  legend.x = NA,
  box.palette = as.list(viridis::viridis(n = 5, direction = -1))
)
...
convert_DCA <- function(data) {
  cut_off <- max(data$age_ce, na.rm = T) - ((max(data$age_ce, na.rm = T) - min(data$age_ce, na.rm = T)) / 2)
...
b_ls_dca_diatoms <-
  ...
  map(~ .x %>% convert_DCA()) %>%
  # map(~mutate(.x, DCA1 = rescale(DCA1, to = c(0,1)))) %>%
  bind_rows() %>%
  select(lake, age_ce, DCA1, DCA2, groups)

# fix Prata because function doesn't work
...
df_dca_multi <-
  bind_rows(b_ls_dca_diatoms, b_ls_dca_chiro, b_ls_dca_fgroup, .id = "troph") %>%
  mutate(group = as.factor(troph))

# make of list for plot list below
ls_dca_multi <-
  df_dca_multi %>% 
  group_split(lake)
...
# Comparisons Diatoms vs Chiro [output as LIST]
...
ls_plot_df_dca_multi_all <- 
  ...
  map2(
    ., names(ls_dca_multi),
    ~ ggplot(.x, aes(x = age_ce, y = DCA1)) +
      geom_smooth(aes(colour = group), linewidth = 2, alpha = 0.5, method = "gam", show.legend = T) +
...
pollen_percent_group <- pollen_normalized %>%
  ...
  group_by(sitename, age_ce) %>%
  mutate(total_scaled = sum(value_scaled),
...
pollen_percent_group <- pollen_percent_group %>%
  mutate(sitename = factor(sitename))  # ensure it exists & is ready
...
# STEP 5: Generate and stack plots
...
# Add fitted values
aug_tree <- augment(mod_tree, type.predict = "response") %>% mutate(group = "Tree")
aug_herb <- augment(mod_herb, type.predict = "response") %>% mutate(group = "Herb")
...
g_norm_abund_guilds <- 
  norm_abund_guilds %>% 
  pivot_longer(cols = `high_profile`:predator,
              names_to = "guild",
              values_to = "rel_abund")
...
gam_models <- guilds %>%
  ...
  map(function(g) {
    ...
    gam(
      rel_abund ~
        s(age_ce, bs = "tp", k = 30) +
        s(age_ce, by = lake, bs = "tp", k = 14, m = 1) +
        s(lake, k = 14, bs = "re"),
      data = df,
      method = "REML"
    )
...
# Check each model for adequacy of k
gam_models_checked <- map(gam_models, mgcv::gam.check)

# Function to capture printed k-check table as a tibble
extract_kcheck_table <- function(model) {
  # Capture the printed output from gam.check()
  txt <- capture.output(gam.check(model, rep = 1000))
  
  # Look for start of the k-check table
  start_line <- grep("Basis dimension \\(k\\) checking results", txt)
  if (length(start_line) == 0) return(NULL)
  
  # Extract lines that make up the table
  table_lines <- txt[(start_line + 2):length(txt)]
  table_lines <- table_lines[table_lines != "" & !grepl("^---", table_lines)]  # remove lines with "---"

  # Read the fixed-width format into a data frame
  read.table(text = table_lines, header = FALSE,
             col.names = c("smooth_term", "k_prime", "edf", "k_index", "p_value"),
...
extract_kcheck_table <- function(model) {
  ...
  table_lines <- txt[(start_line + 2):length(txt)]
  table_lines <- table_lines[table_lines != "" & !grepl("^---", table_lines)]  # remove empty or --- lines

  # Split each line manually
  kcheck_df <- map_dfr(table_lines, function(line) {
    parts <- strsplit(trimws(line), "\\s+")[[1]]
    
    # Handle multi-word smooth term names (e.g., lakeEmpadadas Norte)
    # We assume the last 4 items are always: k', edf, k-index, p-value
...
gam_smooth_plots <- map2(gam_models, guilds, function(model, g) {
  ...
  ggplot(smooths) +
    ...
    geom_point(data = df_res, aes(x = age_ce, y = `s(age_ce)`, color = lake), alpha = 0.5, size = 1) +
...
# Compute effect size difference: Pure Vegetation R² - Pure Climate R²
...
## INTERMEDIATE FIGURE TROPHIC STRUCTURES PLUS POLLEN GAM
```{r fig.width=14, fig.asp=0.35}

# --------------------------------------
# Plot: Vegetation Change (Regional smooth from pollen)
# --------------------------------------
vegetation_smooth_plot <- ggplot(interpolated_df, aes(x = age_ce, y = estimate)) +
  geom_line(size = 1.2) +  # Central smooth line
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci), alpha = 0.2, fill = "steelblue") +  # 95% CI
  theme_minimal(base_size = 14) +
  labs(
    title = "Vegetation Change Regional Smooth",
    x = "Year CE",
    y = "Estimate"
  )
...
# Remove NAs before rescaling
df_p1_smooth <- df_p1_smooth %>%
  filter(!is.na(y)) %>%
...
df_p1_ribbon <- ggplot_build(vegetation_smooth_plot)$data[[2]] %>%
  ...
  mutate(ymin_rescaled = scales::rescale(ymin, to = c(0, 1)),
         ymax_rescaled = scales::rescale(ymax, to = c(0, 1)))

# Get the original range of p1
...
### FIGURE 3D - Combined picture with Pollen GAM
```{r fig.asp = 0.35, fig.width = 14}

# Combine plots with a single collected legend
combined_time_clusts_plus_boxplot_pollen_gam <- 
  p_combined + theme(legend.position = "none") +
  inset_plot + theme(legend.position = "none") +
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'A')


# Save the final figure with appropriate resolution and dimensions
ggsave(plot = combined_time_clusts_plus_boxplot_pollen_gam, 
       filename = "../figures/combined_time_clusts_plus_boxplot_pollen_gam.png", 
       dpi = 600,
       width = 14)
       #height = 12)
...
(fig_1_ord_rf_time_clusts /
  (
    ...
    (inset_plot + 
       theme(legend.position = "none") + 
       ggtitle("(E) Number of species for each community trophic structure"))
  ) +
  plot_layout(guides = "collect", heights = c(8, 5)) +
  plot_annotation(tag_levels = NULL)  #
 )

# Define a theme modifier for consistency
theme_custom <- theme(
  legend.text = element_text(size = 14),        # Increase legend text size
  legend.title = element_text(size = 14),       # Increase legend title size
  axis.text = element_text(size = 13),          # Axis tick labels
  axis.title = element_text(size = 14),          # Axis labels
  plot.margin = unit(c(5, 5, 5, 5), "pt"),  # Top, right, bottom, left
  plot.title = element_text(size = 14, face = "bold"),  # 👈 added line
)

# Apply the theme to each plot
p_combined <- p_combined + theme_custom
inset_plot <- inset_plot + theme_custom
fig_1_ord_rf_time_clusts <- fig_1_ord_rf_time_clusts + theme_custom
...
plot_time.all_in_one <- 
  smooth_all_in_one %>%
  filter(type %in% c("diatoms", "chiro")) %>%
  filter(.smooth == "s(age_ce)") %>%
  ggplot(aes(colour = type), group = type) +
  geom_ribbon(aes(ymin = .lower_ci, ymax = .upper_ci, x = age_ce, colour = type, fill = type),
...
plot_time.all_in_one
ggsave("../figures/FIGURE_1b_azores_map_DCA_two_groups_regional_smooths.svg", dpi = 600, width = 12, height = 6)


```

# FIGURES
### FIGURE 1 - MAP WITH DYNAMICS
```{r fig.asp=0.9, fig.width=12}

# Load lake metadata with UTM coordinates
env <- readxl::read_excel(path = "~/Dropbox/analysis/paleo_trophic_structures/tab_lake_metadata.xlsx")

# Define custom titles with island initials
custom_titles <- c(
  "Azul" = "Azul [SM]",
...
# Modify plot titles in ls_plot_df_dca_multi_all
...
# Remove Furnas and Fogo, then convert to grobs
scatterplots <- 
...
# Load shapefile
coastline_shapefile <- st_read("n _10m_coastline/ne_10m_coastline.shp")
...
panel_b <- ggplot() +
  ...
  geom_sf(data = island_points, aes(geometry = geometry), color = "white", 
          #fill = bg_colors, shape = 21, size = 4) +
  ...
  theme_void() +
  theme(panel.grid.major = element_line(color = "grey70", linetype = "dotted"),
...
for (i in seq_along(scatterplots)) {
  panel_b <- panel_b + 
    annotation_custom(scatterplots[[i]], 
...
ggsave("../figures/FIGURE_1_azores_map_DCA_all_types_two_panels.svg", dpi = 600, width = 12, height = 12)
...
# Save the decision tree plot as an image with higher resolution
png("decision_tree_paleo.png", width = 2000, height = 1400, res = 300)
rpart.plot::rpart.plot(decision_tree_paleo, 
                       type = 5, 
                       extra = 2, 
                       legend.x = NA, 
...
# Read the image and convert it into a ggplot-compatible object
tree_grob <- grid::rasterGrob(png::readPNG("decision_tree_paleo.png"))
plot_decision_tree_paleo <- ggplot() +
  annotation_custom(tree_grob, xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf) +
  theme_void()

# Suppress the "Community Trophic Structures" legend in the ordination plot
...
cts_colors <- c("#FDE725FF", "#7AD151FF", "#22A884FF", "#2A788EFF", "#440154FF")
cts_labels <- c("CTS1", "CTS2", "CTS3", "CTS4", "CTS5")
...
fig_1_ord_rf_time_clusts <-
  ...
  (boxplots + 
     ...
     theme(
       ...
       legend.text = element_text(size = 10),  # Increased legend font
       axis.text = element_text(size = 11),
       axis.title = element_text(size = 13),
...
print(fig_1_ord_rf_time_clusts)

# Save the final figure with appropriate resolution and dimensions
ggsave(plot = fig_1_ord_rf_time_clusts, 
       filename = "../figures/FIGURE_2_ord_rf_time_clusts.png", 
       dpi = 600, 
       width = 12, 
       height = 8)
...
### FIGURE 3B - Community Trophic Structures over time
```{r fig.asp = 1.25, fig.width = 12}
...
main_plot <- fig_2_time_clusts +
  theme(legend.position = "none")  # Remove legend from the main plot

# Create the inset plot
inset_plot <- box_plot_clusts_mod +
  #theme(plot.background = element_rect(             # Add a box around the plot
  #  color = "grey", size = 0.5, fill = NA            # Black border, no fill
  #)) +
guides("none")

# Combine plots using patchwork with inset_element()
combined_fig_2_time_clusts <- main_plot +
  inset_element(inset_plot, 
                left = 0.05, bottom = 0,  # Position on main plot
                right = 0.45, top = 0.4) # Width and height of inset

# Display combined plot
combined_fig_2_time_clusts

# Save the final figure with appropriate resolution and dimensions
ggsave(plot = combined_fig_2_time_clusts, 
       filename = "../figures/combined_FIGURE_3_time_clusts.png", 
       dpi = 600)
       #width = 10, 
       #height = 12)
```

### FIGURE 3C - Community Trophic Structures over time [regional] plus boxplot
```{r fig.asp = 0.5, fig.width = 12}
combined_time_clusts_plus_boxplot <- 
  plot_stacked_fclusts_time_lines + 
  inset_plot +
  patchwork::plot_layout(widths = c(70,30)) +
  plot_layout(guides = "collect")

# Save the final figure with appropriate resolution and dimensions
ggsave(plot = combined_time_clusts_plus_boxplot, 
       filename = "../figures/combined_time_clusts_plus_boxplot.png", 
       dpi = 600,
       width = 14)
       #height = 12)
```
