...
![Community Structure](figures/FIGURE_2_composite.png)
...
`main_script.Rmd` prints `sessionInfo()` at the end to capture the exact package versions used for a run.
...
Two figures are finalised as Adobe Illustrator collages (`.ai` / exported `.pdf`) that combine R-generated panels with illustrations:

- `figures/FIGURE_1_human_impacts.ai`
- `figures/FIGURE_2_composite.ai`

All other figures in `figures/` and `supplementary/` are generated by running `main_script.Rmd`.
